...
#the last part of the list filters a dtm by all three measures(???)
...
dtm_filtered_1<-dfm_select(taz_DTM,
...
applyfilter2 <- function(tokens) {
  #take care of the : - · _ cases, by splitting tokens:
  symbolsplit <- tokens_split(toks, separator = ":",
  ...
  taki<- tokens_compound(symbolsplit, list(c("*", "in"), c("*", "innen"), c("/","in"), 
                                           c("/","innen"), c("(","in",")"),
